...
export default function AdminCommentaires() {
  ...
  const load = async () => {
    let q = supabase.from('commentaires').select('*,sessions(titre),personnages(nom)').order('created_at',{ascending:false})
    ...
    <AdminLayout title="Commentaires">
      ...
      {commentaires.length === 0 && (
        <div className="card" style={{ padding:'3rem', textAlign:'center' }}>
          ...
          <p style={{ color:'var(--ash)' }}>{filtre==='attente'?'Aucun commentaire en attente.':filtre==='approuves'?'Aucun commentaire approuvé.':'Aucun commentaire.'}</p>
        </div>
      )}
        ...
        {commentaires.map(c => (
          <div key={c.id} className="card" style={{ padding:'1.5rem', borderColor:c.approuve?'rgba(40,120,40,0.3)':'rgba(201,168,76,0.3)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'0.75rem', marginBottom:'0.75rem' }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', flexWrap:'wrap', marginBottom:'0.3rem' }}>
                  ...
                  {c.personnages && <span style={{ color:'var(--ash)', fontSize:'0.8rem' }}>🧙 {c.personnages.nom}</span>}
            ...
            {c.contenu ? (
              ...
              <p style={{ color:'var(--ash)', fontSize:'0.85rem', fontStyle:'italic', margin:0 }}>Note uniquement, sans commentaire écrit.</p>
            )}
          </div>
        ))}
      </div>
